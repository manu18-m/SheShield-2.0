// src/App.jsx — SheShield v1.0 (College Project Edition)
...
:root{
  --bg:#07070f;--s1:#0d0d1a;--s2:#131324;--s3:#1a1a2e;--s4:#22223c;
...
.sbar{height:50px;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:space-between;padding:0 28px;flex-shrink:0;font-size:12px;font-weight:700;position:relative;z-index:10}
...
.nb{display:flex;flex-direction:column;align-items:center;gap:4px;padding:8px 20px;border-radius:18px;cursor:pointer;border:none;background:none;color:var(--muted);font-family:var(--fb);font-size:10px;font-weight:600;transition:all .2s}
...
.btn{width:100%;padding:15px;background:var(--rose);border:none;border-radius:var(--rs2);color:#fff;font-family:var(--fh);font-size:16px;font-weight:600;cursor:pointer;transition:all .2s;box-shadow:0 8px 24px var(--rg);margin-top:6px;display:flex;align-items:center;justify-content:center;gap:8px}
.btn:hover:not(:disabled){transform:translateY(-1px)}
.btn:disabled{opacity:.5;cursor:not-allowed}
.btnghost{background:var(--s3);box-shadow:none;color:var(--txt)}
.btnghost:hover:not(:disabled){background:var(--s4);transform:none}
...
.chs{background:var(--ts);color:var(--teal)}.chw{background:var(--as2);color:var(--amber)}.chd{background:var(--rs);color:var(--rose)}
...
.sbar2.safe{background:var(--ts);border-color:rgba(0,212,170,.2)}.sbar2.sos{background:var(--rs);border-color:rgba(255,31,107,.25)}
.sdot{width:8px;height:8px;border-radius:50%;flex-shrink:0}
.sdot.safe{background:var(--teal);animation:pt 2s infinite}.sdot.sos{background:var(--rose);animation:pr .75s infinite}
...
.lname{font-size:14px;font-weight:600}.lcoord{font-size:11px;color:var(--muted);margin-top:2px}
...
.moodbtn:hover{transform:translateY(-2px)}.moodbtn.sel{border-color:var(--rose)}
.me{font-size:20px}.ml{font-size:10px;font-weight:600;color:var(--dim)}
...
.wtime{font-family:var(--fh);font-size:26px;font-weight:700}.wsub{font-size:12px;color:var(--dim);margin-top:2px}
...
.sosring.act:nth-child(2){border-color:rgba(255,31,107,.3)}.sosring.act:nth-child(3){border-color:rgba(255,31,107,.15)}
...
/* FAKE CALL - fixed with position:fixed so it always shows on top */
.fcall{position:fixed;inset:0;z-index:99999;background:linear-gradient(180deg,#091a10,#040c08);display:flex;flex-direction:column;align-items:center;justify-content:space-between;padding:80px 30px 70px}
...
.fcwave span:nth-child(1){height:8px}.fcwave span:nth-child(2){height:16px;animation-delay:.15s}.fcwave span:nth-child(3){height:12px;animation-delay:.3s}.fcwave span:nth-child(4){height:18px;animation-delay:.45s}.fcwave span:nth-child(5){height:10px;animation-delay:.6s}
...
.fcend{background:var(--rose);box-shadow:0 8px 28px var(--rg)}.fcans{background:var(--teal);box-shadow:0 8px 28px var(--tg)}
.fclbl{font-size:12px;color:rgba(255,255,255,.5)}
...
.ccard{background:var(--s2);border:1px solid var(--brd);border-radius:var(--rs2);padding:13px;margin-bottom:8px;display:flex;align-items:center;gap:12px}
...
.cinfo{flex:1;min-width:0}.cname{font-weight:600;font-size:14px;margin-bottom:1px}.cph{font-size:12px;color:var(--dim)}.crel{font-size:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--muted);margin-top:2px}
...
.cbtn:hover{color:var(--rose)}.cbtn.on{background:var(--ts);border-color:rgba(0,212,170,.4);color:var(--teal)}
...
.ictime{font-size:11px;color:var(--muted)}.icnote{font-size:13px;color:var(--dim);line-height:1.5}
...
.pname{font-family:var(--fh);font-size:18px;font-weight:700}.pemail{font-size:12px;color:rgba(255,255,255,.45);margin-top:2px}.pbadge{font-size:11px;color:rgba(0,212,170,.8);margin-top:4px}
...
.srow:last-child{border-bottom:none}.srow:hover{background:var(--rs)}
...
/* CALCULATOR - fixed with position:fixed */
.calcwrap{position:fixed;inset:0;z-index:99999;background:#1c1c1e;display:flex;flex-direction:column}
.calcd{flex:1;display:flex;flex-direction:column;align-items:flex-end;justify-content:flex-end;padding:20px 24px 12px;gap:6px}
.calce{font-size:20px;color:#888;font-family:monospace;word-break:break-all;text-align:right;min-height:28px}
.calcr{font-size:56px;font-weight:200;font-family:monospace;color:#fff;word-break:break-all;line-height:1;text-align:right}
.calcg{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;padding:12px 16px 36px}
.ck{height:68px;border-radius:50%;border:none;font-size:24px;cursor:pointer;transition:all .12s;display:flex;align-items:center;justify-content:center;font-family:inherit}
.ck:active{opacity:.55;transform:scale(.91)}
.ckfn{background:#333;color:#fff}.ckop{background:#ff9f0a;color:#fff}.cknum{background:#505050;color:#fff}
.ck0{border-radius:34px!important;width:100%;justify-content:flex-start;padding-left:26px}
.calch{text-align:center;font-size:11px;color:#3a3a3a;padding-bottom:16px}
`;
...
export default function App() {
  ...
  const [cnew,  setCnew]  = useState(true);
  const [cbuf,  setCbuf]  = useState(""); // secret code buffer
  ...
  useEffect(() => {
    const u = onAuthChange((user) => { setAuthUser(user); setAuthLoading(false); if (user) setPage("app"); });
    return u;
  }, []);
  ...
  useEffect(() => { setDanger(hour()>=21||hour()<6?45:18); }, []);
  ...
  const cancelSOS = useCallback(() => {
    ...
    stopAlarm(); toast("✓ SOS cancelled. You're safe!");
  }, [stopAlarm]);
  ...
  const doLogin   = async () => { setAerr(""); setBusy(true); try { await logIn(lf.email,lf.password); } catch(e){ setAerr(friendlyError(e.code)); } finally{ setBusy(false); } };
  const doSignup  = async () => {
    ...
    try { await signUp(sf.name,sf.email,sf.password); } catch(e){ setAerr(friendlyError(e.code)); } finally{ setBusy(false); }
  };
  const doForgot  = async () => {
    if(!fe){ setAerr("Enter your email."); return; }
    setAerr(""); setBusy(true);
    try { await resetPassword(fe); toast("✅ Reset email sent!"); setPage("login"); } catch(e){ setAerr(friendlyError(e.code)); } finally{ setBusy(false); }
  };
  const doLogout  = async () => { await logOut(); setAuthUser(null); setPage("login"); setSosActive(false); stopAlarm(); };
  ...
  const triggerFake = (caller, delay=0) => {
    ...
    fakeRef.current = setTimeout(()=>setFakeOn(false), 60000);
  };

  // ── Calculator — completely rewritten and fixed ────────────
  const calcReset = () => { setCval("0"); setCexpr(""); setCprev(""); setCop(""); setCnew(true); setCbuf(""); };

  const calcPress = (v) => {
    // Secret unlock: track last 4 digits pressed
    if (/^\d$/.test(v)) {
      const next = (cbuf + v).slice(-4);
      setCbuf(next);
      if (next === "1234") { setDisc(false); calcReset(); return; }
    }

    if (v === "AC")  { calcReset(); return; }
    if (v === "⌫")   { setCval(p => p.length>1 ? p.slice(0,-1) : "0"); return; }
    if (v === "%")   { setCval(p => String(parseFloat(p)/100)); return; }
    if (v === "+/-") { setCval(p => p.startsWith("-") ? p.slice(1) : p==="0" ? "0" : "-"+p); return; }

    if (["+","-","×","÷"].includes(v)) {
      const opMap={"×":"*","÷":"/"};
      const realOp = opMap[v]||v;
      if (cop && !cnew) {
        try {
          const r = Function(`"use strict";return(${cprev}${cop}${cval})`)();
          const rs = isNaN(r)||!isFinite(r) ? "Error" : parseFloat(r.toFixed(10)).toString();
          setCprev(rs); setCval(rs); setCexpr(rs+" "+v);
        } catch { setCexpr(cval+" "+v); setCprev(cval); }
      } else { setCprev(cval); setCexpr(cval+" "+v); }
      setCop(realOp); setCnew(true); return;
    }
    ...
    if (v === "=") {
      ...
      try {
        const r = Function(`"use strict";return(${cprev}${cop}${cval})`)();
        const rs = isNaN(r)||!isFinite(r) ? "Error" : parseFloat(r.toFixed(10)).toString();
        const opSym = cop==="*"?"×":cop==="/"?"÷":cop;
        setCexpr(cprev+" "+opSym+" "+cval+" =");
        setCval(rs); setCprev(""); setCop(""); setCnew(true);
      } catch { setCval("Error"); setCnew(true); }
      return;
    }

    if (v===".") {
      if(cnew){ setCval("0."); setCnew(false); return; }
      if(!cval.includes(".")) setCval(p=>p+"."); return;
    }

    // Digit
    if(cnew){ setCval(v); setCnew(false); }
    else { setCval(p => p==="0"?v : p.length>=12?p : p+v); }
  };
  ...
  if (authLoading) return (
    <><style>{CSS}</style>
    <div className="shell"><div className="scr"><div className="loadscr">
      ...
      <div style={{fontFamily:"var(--fh)",fontSize:22,fontWeight:700}}>She<span style={{color:"var(--rose)"}}>Shield</span></div>
      <div style={{display:"flex",alignItems:"center",gap:8,color:"var(--dim)",fontSize:13}}><span className="spin"/>Loading…</div>
    </div></div></div></>
  );

  // ── Discreet Calculator — fixed ────────────────────────────
  if (disc) {
    ...
    return (
      ...
      <div className="calcwrap">
        <div className="calcd">
          <div className="calce">{cexpr || " "}</div>
          <div className="calcr">{cval}</div>
        </div>
        <div className="calcg">
          {rows.map((row,ri)=>row.map((k,ki)=>{
            ...
            return (
              <button key={`${ri}${ki}`}
                className={`ck ${isOp?"ckop":isFn?"ckfn":"cknum"}${isZ?" ck0":""}`}
                style={isZ?{gridColumn:"span 2"}:{}}
                onClick={()=>calcPress(k)}>
                {k}
              </button>
            );
        ...
        <div className="calch">Type 1234 to return to SheShield</div>
      </div></>
    );
  }
  ...
  return (
    ...
    {/* Fake Call — position:fixed, renders outside shell, always on top */}
    {fakeOn&&(
      <div className="fcall">
        ...
        <div style={{textAlign:"center",color:"rgba(255,255,255,.4)",fontSize:13}}>Use this to exit uncomfortable situations</div>
        <div className="fcbtns">
          <div className="fcgrp">
            <button className="fcbtn fcend" onClick={()=>{setFakeOn(false);clearTimeout(fakeRef.current);}}>
              <IcPhone size={28} color="#fff" style={{transform:"rotate(135deg)"}}/>
            </button>
            <span className="fclbl">Decline</span>
          </div>
          <div className="fcgrp">
            <button className="fcbtn fcans" onClick={()=>{toast("Stay safe! 💗");setFakeOn(false);clearTimeout(fakeRef.current);}}>
              <IcPhone size={28} color="#fff"/>
            </button>
            <span className="fclbl">Answer</span>
          </div>
        </div>
      </div>
    )}
    ...
    <div className="shell">
      ...
      <div className="scr">
        ...
        {page==="login"&&(
          <div className="auth">
            <div className="alogo"><div className="alogobox"><IcShield size={24}/></div><div className="alogoname">She<span>Shield</span></div></div>
            <div className="atitle">Welcome<br/>back 💗</div>
            <div className="asub">Your personal safety companion.</div>
              ...
              <div className="sovlay">
                ...
                <div className="sovlist">
                  {[
                    ...
                    {e:"📱",t:actC.length>0?`Contacts: ${actC.map(c=>c.name).join(", ")}`:"⚠ No active contacts"},
                    {e:"🔊",t:buzzer?"Alarm buzzer active":"Buzzer off"},
                    {e:"🗺️",t:"Google Maps link ready"},
                  ].map((r,i)=>(
            ...
            {subTab==="incidents"&&(
              <div className="pg">
                ...
                <div style={{background:"rgba(124,111,255,.08)",border:"1px solid rgba(124,111,255,.2)",borderRadius:"var(--r)",padding:"12px 14px",marginBottom:14,fontSize:13,lineHeight:1.55}}>
                  <b style={{color:"var(--violet)"}}>Private & secure.</b> Each entry is timestamped + GPS logged.
                </div>
                  ...
                  <div className="icard" key={inc.id}>
                    <div className="ichdr">
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <span className="ictime">{inc.createdAt ? new Date(inc.createdAt).toLocaleString() : "Just now"}</span>
              ...
              <div style={{paddingBottom:24}}>
                ...
                <div className={`sbar2 ${sosActive?"sos":"safe"}`}>
                  ...
                  <div className="sb2t" style={{color:sosActive?"var(--rose)":"var(--teal)"}}>
                    {sosActive?"🚨 SOS Active — Alerting contacts…":"✓ You are safe · Live GPS tracking on"}
                  </div>
                ...
                <div className="wcard">
                  <div className="whdr">
                    ...
                    <div className="wbadge" style={{background:walkOn?"var(--rs)":"var(--as2)",color:walkOn?"var(--rose)":"var(--amber)"}}>{walkOn?"ACTIVE":"SET"}</div>
                  </div>
                  ...
                  <div className="qgrid">
                    {[
                      ...
                      {e:"📲",l:"Alert\nAll",     bg:"#0a1a18",c:"#00d4aa",a:()=>{
                        ...
                        const msg=encodeURIComponent(gps.getSOSMessage?.() ?? "🆘 I need help!");
                      ...
                      {e:"📍",l:"Copy\nLocation", bg:"#0a1220",c:"#06b6d4",a:()=>{
                        ...
                        navigator.clipboard?.writeText(msg).then(()=>toast("✅ SOS message copied!")).catch(()=>toast("⚠ Copy failed — try on mobile"));
                      }},
                      ...
                      {e:"🎤",l:"Voice\nHelp",    bg:"#1a0a18",c:"#ec4899",a:()=>{
                        ...
                        const r=new SR(); r.start(); toast("🎤 Say 'help' or 'bachao'…");
                        r.onresult=(e)=>{ const w=e.results[0][0].transcript.toLowerCase(); if(w.includes("help")||w.includes("bachao")||w.includes("sos")){ triggerSOS(); } else { toast(`Heard: "${w}"`); } };
                        r.onerror=()=>toast("⚠ Mic access denied");
                      }},
                      {e:"🏥",l:"Ambulance\n108", bg:"#0a150a",c:"#22c55e",a:()=>{ window.location.href="tel:108"; }},
                      {e:"🔥",l:"Fire\n101",      bg:"#1a1008",c:"#ff8800",a:()=>{ window.location.href="tel:101"; }},
                      {e:"🕵️",l:"Discreet\nMode", bg:"#0a0a1a",c:"#8888cc",a:()=>{ calcReset(); setDisc(true); toast("Calculator mode. Type 1234 to return."); }},
                    ].map((a,i)=>(
                  ...
                  return (
                    <div className="ccard" key={c.id}>
                      ...
                      <div className="cbtns">
                        ...
                        <button className="cbtn" style={{color:"var(--rose)"}} onClick={()=>deleteContact(c.id).then(()=>toast("Contact removed."))}><IcTrash size={13}/></button>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="pg">
                ...
                <div className="sgrp">
                  {[
                    ...
                    {e:"🕵️",l:"Discreet Mode",r:<button className="toggle" onClick={()=>{calcReset();setDisc(true);toast("Calculator mode. Type 1234 to return.");}}/>},
                  ].map((r,i)=>(
                ...
                <div style={{textAlign:"center",color:"var(--muted)",fontSize:12,marginTop:16,paddingBottom:8,lineHeight:1.8}}>
                  SheShield v1.0 — Women Safety App<br/>
                  Firebase Auth · GPS · Web Audio API<br/>
              ...
              <div className="mbd" onClick={()=>setShowIM(false)}>
                <div className="msh" onClick={e=>e.stopPropagation()}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:7,marginBottom:14}}>
                    {INC_TYPES.map(t=>(
                      <button key={t} style={{padding:"7px 14px",borderRadius:"100px",border:`1.5px solid ${ni.type===t?"var(--rose)":"var(--brd)"}`,background:ni.type===t?"var(--rs)":"var(--s3)",color:ni.type===t?"var(--rose)":"var(--dim)",cursor:"pointer",fontFamily:"var(--fb)",fontSize:13,fontWeight:600}} onClick={()=>setNi(p=>({...p,type:t}))}>{t}</button>
                    ))}
                  </div>
                  ...
                  <div style={{display:"flex",gap:8,marginBottom:14}}>
                    {[{v:"low",l:"🟢 Low",c:"var(--teal)"},{v:"med",l:"🟡 Medium",c:"var(--amber)"},{v:"high",l:"🔴 High",c:"var(--rose)"}].map(s=>(
                      <button key={s.v} style={{flex:1,padding:"10px",border:`1.5px solid ${ni.severity===s.v?s.c:"var(--brd)"}`,borderRadius:"var(--rs2)",background:ni.severity===s.v?`${s.c}18`:"var(--s3)",color:ni.severity===s.v?s.c:"var(--dim)",cursor:"pointer",fontFamily:"var(--fb)",fontSize:13,fontWeight:600}} onClick={()=>setNi(p=>({...p,severity:s.v}))}>{s.l}</button>
                    ))}
                  </div>
